...
import { Users, Clock, AlertCircle, CheckCircle2, Monitor, MoreVertical, KeyRound, Power, Server, Search, Filter, ArrowLeft, ArrowUpDown, ChevronUp, ChevronDown } from 'lucide-react';
...
type SortKey = 'machineId' | 'name' | 'studentId' | 'status' | 'progress' | 'score';
...
export default function InvigilatorPage() {
  ...
  // Action Modals State
  const [resetSessionStudent, setResetSessionStudent] = useState<StudentStatus | null>(null);
  const [resetPasswordStudent, setResetPasswordStudent] = useState<StudentStatus | null>(null);
  ...
  const [submitFilter, setSubmitFilter] = useState<'all' | 'submitted' | 'not'>('all');
  ...
  const getStatusBadge = (status: StudentStatus['status']) => {
    switch (status) {
      case 'OFFLINE':
        return <span className="px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-slate-100 text-slate-700 dark:bg-slate-500/15 dark:text-slate-300">Chưa đăng nhập</span>;
      case 'READY':
        return <span className="px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300">Đã sẵn sàng</span>;
      case 'IN_PROGRESS':
        return <span className="px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300 flex items-center gap-1 w-fit"><Monitor className="w-3 h-3"/> Đang làm bài</span>;
      case 'SUBMITTED':
        return <span className="px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300 flex items-center gap-1 w-fit"><CheckCircle2 className="w-3 h-3"/> Đã nộp bài</span>;
    }
  };

  const renderProgressBar = (progress?: { completed: number; total: number }, status?: string) => {
    if (!progress || status === 'OFFLINE') return <span className="text-xs text-[var(--text-muted)]">-</span>;
    ...
    return (
      <div className="flex flex-col gap-1 w-24">
        <div className="flex justify-between text-[10px] font-bold">
          <span className={isSubmitted ? 'text-emerald-600' : 'text-brand-600'}>{progress.completed}/{progress.total}</span>
  ...
  if (isLoading) {
    return <div className="animate-pulse flex flex-col gap-4"><div className="h-24 bg-slate-200 dark:bg-slate-800 rounded-xl"></div></div>;
  }
  ...
  const filteredRooms = rooms.filter(r => {
    ...
    if (filterShift !== 'all' && r.shift.toString() !== filterShift) return false;
    if (filterSubject !== 'all' && filterSubject !== '' && r.examName !== filterSubject) return false;
    return true;
  });
  ...
  const STATUS_ORDER: Record<StudentStatus['status'], number> = {
    IN_PROGRESS: 0, SUBMITTED: 1, READY: 2, OFFLINE: 3,
  };
  const visibleStudents = students
    .filter((s) => {
      if (submitFilter === 'submitted' && s.status !== 'SUBMITTED') return false;
    ...
    .sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'studentId': r = a.studentId.localeCompare(b.studentId); break;
        case 'status': r = STATUS_ORDER[a.status] - STATUS_ORDER[b.status]; break;
        case 'progress': r = (a.progress?.completed ?? 0) - (b.progress?.completed ?? 0); break;
        case 'score': r = (a.score ?? -1) - (b.score ?? -1); break;
        default: r = (a.machineId ?? '').localeCompare(b.machineId ?? '');
      }
      return sortAsc ? r : -r;
    });
  ...
  const uniqueShifts = Array.from(new Set(rooms.map(r => r.shift))).sort((a, b) => a - b);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        ...
        <GlassCard className="p-4 flex flex-col md:flex-row gap-4 items-end bg-[var(--bg-glass)] shadow-sm">
          <div className="flex-1 flex flex-col gap-1.5 w-full">
            <label className="text-[13px] font-bold text-[var(--text-secondary)]">Ngày thi</label>
            <input 
              type="date"
          ...
          <div className="flex-1 flex flex-col gap-1.5 w-full">
            <label className="text-[13px] font-bold text-[var(--text-secondary)]">Ca / Tiết thi</label>
            <select 
              value={filterShift} 
            ...
            >
              ...
              {uniqueShifts.map(s => <option key={s} value={s.toString()}>Ca {s}</option>)}
            </select>
          </div>

          <div className="flex-[2] flex flex-col gap-1.5 w-full">
            <label className="text-[13px] font-bold text-[var(--text-secondary)]">Tên môn thi / Mã học phần</label>
            <select 
              value={filterSubject} 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredRooms.map(room => (
          ...
          >
            ...
            <p className="text-sm font-medium text-[var(--text-secondary)] mb-3">{room.examName}</p>
            <div className="flex items-center justify-between text-xs text-[var(--text-muted)] border-t border-[var(--border-subtle)] pt-3">
              <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5"/> {room.totalStudents} SV</span>
              <span className="flex items-center gap-1 font-semibold text-brand-600 dark:text-blue-400">Ca {room.shift}</span>
              <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5"/> {room.examDate}</span>
            </div>
          </GlassCard>
        ))}
        {filteredRooms.length === 0 && (
          <div className="col-span-full py-12 flex flex-col items-center justify-center text-[var(--text-muted)] bg-[var(--bg-glass-light)] rounded-[20px] border border-dashed border-[var(--border-subtle)]">
            ...
            <p className="font-medium">Không tìm thấy phòng thi nào phù hợp với bộ lọc.</p>
          </div>
        )}
      </div>
      </>)}
      ...
      {selectedRoom && (
        <div className="flex flex-col gap-5">
          <button
            onClick={() => { setSelectedRoom(null); setStudents([]); }}
          ...
          {selectedRoomData && (
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                <h2 className="text-2xl font-bold text-[var(--text-primary)]">{selectedRoomData.name}</h2>
                <p className="text-[14px] text-[var(--text-secondary)] mt-0.5">
                  {selectedRoomData.examName} · Ca {selectedRoomData.shift} · {selectedRoomData.totalStudents} SV
                </p>
              </div>
            </div>
          )}
          ...
          <div className="p-5 border-b border-[var(--border-subtle)] bg-[var(--bg-glass-light)] flex flex-col gap-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <h3 className="text-lg font-bold text-[var(--text-primary)]">Danh sách Sinh viên</h3>
              <div className="flex gap-4 text-sm font-medium">
                <div className="flex items-center gap-2 text-emerald-600"><CheckCircle2 className="w-4 h-4"/> Đã nộp: {students.filter(s => s.status === 'SUBMITTED').length}</div>
                <div className="flex items-center gap-2 text-amber-600"><Monitor className="w-4 h-4"/> Đang thi: {students.filter(s => s.status === 'IN_PROGRESS').length}</div>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <select
                ...
                value={submitFilter}
                onChange={(e) => setSubmitFilter(e.target.value as 'all' | 'submitted' | 'not')}
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="bg-[var(--bg-glass)] border-b border-[var(--border-subtle)] text-[12px] font-semibold text-[var(--text-secondary)] uppercase tracking-wider">
                  <th className="p-4 pl-6"><SortTh k="machineId" label="Máy / IP" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} /></th>
                  <th className="p-4"><SortTh k="name" label="Thông tin Sinh viên" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} /></th>
                  <th className="p-4"><SortTh k="status" label="Trạng thái" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} /></th>
                  <th className="p-4"><SortTh k="progress" label="Tiến độ" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} /></th>
                  <th className="p-4"><SortTh k="score" label="Điểm" sortKey={sortKey} sortAsc={sortAsc} onSort={toggleSort} /></th>
              ...
              <tbody className="divide-y divide-[var(--border-subtle)] text-[13px]">
                {pagedStudents.map(student => (
                  <tr key={student.id} className="hover:bg-[var(--bg-glass-light)] transition-colors group">
                    <td className="p-4 pl-6">
                      {student.machineId ? (
                        <div className="flex flex-col">
                          <span className="text-[13px] font-semibold text-brand-600 dark:text-blue-400 flex items-center gap-1.5"><Server className="w-3.5 h-3.5 text-brand-500" /> {student.machineId}</span>
                          <span className="text-[11px] font-mono text-[var(--text-muted)] mt-0.5">{student.ipAddress}</span>
                        </div>
                      ) : (
                        <span className="text-xs text-[var(--text-muted)] italic">Chưa kết nối</span>
                      )}
                    </td>
                    <td className="p-4">
                      <div className="flex flex-col">
                        <span className="text-[13px] font-medium text-[var(--text-primary)]">{student.name}</span>
                        <span className="text-xs text-[var(--text-secondary)]">{student.studentId}</span>
                        <span className="text-[11px] text-[var(--text-muted)] font-mono mt-0.5">{student.phone}</span>
                      </div>
                    </td>
                    <td className="p-4">{getStatusBadge(student.status)}</td>
                    <td className="p-4">{renderProgressBar(student.progress, student.status)}</td>
                    <td className="p-4">
                      {student.status === 'SUBMITTED' && student.score != null ? (
                        <span className="text-[14px] font-bold text-emerald-600">{student.score.toFixed(2)}</span>
                      ) : (
                        <span className="text-xs text-[var(--text-muted)]">-</span>
                      )}
                    </td>
                    <td className="p-4">
                      {liveTime(student) && student.status === 'IN_PROGRESS' ? (
                    ...
                    <td className="p-4 text-center">
                      {student.violationCount > 0 ? (
                        <span className="inline-flex items-center justify-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-rose-100 text-rose-700 dark:bg-rose-500/15 dark:text-rose-300">
                          <AlertCircle className="w-3 h-3" /> {student.violationCount}
                        </span>
                      ) : (
                        <span className="text-xs text-[var(--text-muted)]">-</span>
                      )}
                    ...
                    <td className="p-4 pr-6 text-right relative">
                      ...
                      {openDropdownId === student.id && (
                        ...
                        >
                          ...
                          >
                            <KeyRound className="w-4 h-4 text-amber-500" /> Cấp lại mật khẩu
                          </button>
                          <button 
                ...
                {visibleStudents.length === 0 && (
                  <tr>
                    <td colSpan={8} className="p-8 text-center text-[var(--text-secondary)]">
                      {students.length === 0 ? 'Không có sinh viên nào.' : 'Không tìm thấy sinh viên khớp tìm kiếm.'}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {visibleStudents.length > PAGE_SIZE && (
            <div className="p-4 border-t border-[var(--border-subtle)] flex items-center justify-between bg-[var(--bg-glass-light)] text-[13px]">
              <span className="text-[var(--text-secondary)]">
                Hiển thị <b className="text-[var(--text-primary)]">{(safePage - 1) * PAGE_SIZE + 1}–{Math.min(safePage * PAGE_SIZE, visibleStudents.length)}</b> / {visibleStudents.length} sinh viên
              </span>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[var(--text-secondary)] font-medium px-1">Trang {safePage}/{pageCount}</span>
                <button
      ...
      {resetPasswordStudent && selectedRoom && (
        <ResetPasswordModal
          ...
          onGenerate={() =>
            resetStudentPassword(selectedRoom, resetPasswordStudent.studentId).then(
              (r) => r.newPassword,
            )
          }
        />
