...
export function SubmitConfirmModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="p-0 overflow-hidden max-w-lg">
      <div className="p-8">
        <div className="flex flex-col items-center text-center">
          ...
          {isAllAnswered ? (
            <p className="text-[15px] text-[var(--text-secondary)] mb-8">
              Tuyệt vời! Bạn đã hoàn thành toàn bộ <strong>{totalQuestions}</strong> câu hỏi. Bạn có chắc chắn muốn nộp bài ngay bây giờ?
            </p>
          ) : (
            <div className="mb-8">
              <p className="text-[15px] text-[var(--text-secondary)] mb-4">
                Bạn vẫn còn <strong>{unansweredCount}</strong> câu hỏi chưa chọn đáp án. Bạn có thực sự muốn nộp bài lúc này?
              </p>
              <div className="flex items-center justify-center gap-4 text-[14px]">
                <div className="flex flex-col items-center p-3 rounded-xl bg-[var(--bg-glass-light)] border border-[var(--border-subtle)] w-28">
                  <span className="font-bold text-emerald-500 text-lg">{answeredCount}</span>
                  <span className="text-[11px] text-[var(--text-muted)] uppercase tracking-wide">Đã làm</span>
                </div>
                <div className="flex flex-col items-center p-3 rounded-xl bg-[var(--bg-glass-light)] border border-amber-500/30 w-28">
                  <span className="font-bold text-amber-500 text-lg">{unansweredCount}</span>
                  <span className="text-[11px] text-[var(--text-muted)] uppercase tracking-wide">Chưa làm</span>
                </div>
              </div>
            </div>
          )}
