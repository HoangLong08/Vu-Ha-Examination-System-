...
export function ResetPasswordModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Cấp lại mật khẩu" className="max-w-md p-6">
        ...
        {!newPassword ? (
          <>
            <p className="text-[13px] text-[var(--text-muted)] mb-6 px-4">
              Mật khẩu mới sẽ được đặt bằng <strong>mã số sinh viên</strong> để thí sinh đăng nhập lại vào kỳ thi.
            </p>
            {error && (
              <p className="text-[13px] font-medium text-rose-600 mb-4">{error}</p>
            )}
          ...
          <div className="w-full flex flex-col items-center">
            <p className="text-[13px] text-[var(--text-secondary)] mb-3">Mật khẩu mới đã được tạo thành công:</p>
            ...
            <p className="text-[12px] text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-3 py-2 rounded-lg mb-6 w-full text-left">
              Vui lòng đọc hoặc gửi mật khẩu này cho sinh viên. Mật khẩu cũ đã bị vô hiệu hóa.
            </p>
            <Button variant="primary" onClick={handleClose} className="w-full py-2.5">
              Hoàn tất
            </Button>
          </div>
        )}
